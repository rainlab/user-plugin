<?php
return [
    "account" => [
        "account" => "帐号",
        "account_desc" => "用户信息.",
        "activation_by_admin" => "",
        "activation_code" => "激活码",
        "activation_code_placeholder" => "输入激活码",
        "activation_email_sent" => "激活邮件已发送至您的邮箱.",
        "already_active" => "您的帐号暂未激活!",
        "banned" => "",
        "change_details" => "要更改这些详细信息，请确认您的当前密码。",
        "code_param" => "激活码参数。",
        "code_param_desc" => "激活码的验证页面URL参数",
        "current_password" => "当前密码",
        "deactivate" => "停用您的帐户？",
        "deactivate_change" => "我改变主意了",
        "deactivate_confirm" => "确认停用帐户",
        "deactivate_notice" => "您的帐户将被禁用，您的详细信息将从网站中删除。\n您可以随时重新登录以重新激活您的帐户。",
        "deactivate_password" => "要继续，请输入密码：",
        "deactivate_toggle" => "关闭户口",
        "email" => "邮箱",
        "email_not_verified" => "您的电子邮件地址尚未验证。",
        "email_placeholder" => "输入你的电子邮箱",
        "email_resend" => "再次发送验证电子邮件",
        "email_verify" => "您应该验证您的帐户，否则它可能会被删除。请检查您的电子邮件以进行验证。",
        "enter_your" => "输入您的",
        "force_secure" => "",
        "force_secure_desc" => "",
        "full_name" => "全名",
        "full_name_placeholder" => "输入您的全名",
        "invalid_activation_code" => "错误的激活码",
        "invalid_current_pass" => "",
        "invalid_deactivation_pass" => "密码不正确。",
        "invalid_user" => "未找到该用户。",
        "login" => "登录",
        "login_first" => "您需要先登录帐号才能访问该页面!",
        "new_password" => "设置密码",
        "new_password_confirm" => "确认密码",
        "new_password_placeholder" => "输入新的密码",
        "password" => "密码",
        "password_placeholder" => "选择一个密码",
        "password_reset" => "重设密码",
        "password_reset_complete" => "密码重置完成，您现在可以登录。",
        "redirect_to" => "跳转至",
        "redirect_to_desc" => "登录或注册成功后跳转页面.",
        "register" => "注册",
        "registration_disabled" => "当前已经禁用注册。",
        "registration_throttled" => "",
        "required" => "*必填",
        "save" => "保存",
        "sign_in" => "登录",
        "stay_logged_in" => "保持登录状态",
        "success_activation" => "您的帐号已成功激活.",
        "success_deactivation" => "成功停用您的账号，很抱歉看到您离我而去!",
        "success_saved" => "设置保存成功!",
        "update_requires_password" => "",
        "update_requires_password_comment" => ""
    ],
    "country" => ["code" => "", "code_comment" => "", "enabled" => "", "label" => "", "name" => ""],
    "group" => [
        "code" => "编码",
        "code_comment" => "输入一个唯一编码来标志当前用户组",
        "created_at" => "创建",
        "description_field" => "描述",
        "id" => "ID",
        "is_new_user_default_field" => "",
        "label" => "群组",
        "name" => "姓名",
        "users_count" => "用户"
    ],
    "groups" => [
        "all_groups" => "用户群组",
        "create_title" => "创建群组",
        "delete_confirm" => "确定要删除该群组吗？",
        "delete_selected_confirm" => "确定清空选中用户组么？",
        "delete_selected_empty" => "没有选中的要删除的群组。",
        "delete_selected_success" => "成功删除选中的群组。",
        "list_title" => "管理群组",
        "menu_label" => "群组",
        "new_group" => "新的用户组",
        "preview_title" => "上一个群组",
        "return_to_list" => "返回群组列表",
        "return_to_users" => "返回用户列表",
        "update_title" => "编辑群组"
    ],
    "location" => [
        "create_title" => "",
        "label" => "",
        "new" => "",
        "preview_title" => "",
        "update_title" => ""
    ],
    "locations" => [
        "delete_confirm" => "",
        "disable_confirm" => "",
        "disable_success" => "",
        "enable_or_disable" => "",
        "enable_or_disable_title" => "",
        "enable_success" => "",
        "enabled_help" => "",
        "enabled_label" => "",
        "hide_disabled" => "",
        "list_title" => "",
        "menu_description" => "",
        "menu_label" => "",
        "return_to_list" => "",
        "selected_amount" => ""
    ],
    "login" => ["attribute_email" => "邮箱", "attribute_username" => "用户名"],
    "plugin" => [
        "access_groups" => "管理用户组",
        "access_settings" => "管理用户设置",
        "access_users" => "管理用戶",
        "description" => "用户管理.",
        "impersonate_user" => "",
        "name" => "用户",
        "tab" => "用户"
    ],
    "reset_password" => [
        "code_param" => "重置验证码参数",
        "code_param_desc" => "重置密码的验证码页面URL参数",
        "reset_password" => "重置密码",
        "reset_password_desc" => "找回密码."
    ],
    "session" => [
        "all" => "所有人",
        "allowed_groups_description" => "",
        "allowed_groups_title" => "",
        "guests" => "游客",
        "logout" => "你已经成功退出登陆！",
        "redirect_desc" => "拒绝访问时重定向到页面的名字。",
        "redirect_title" => "跳转至",
        "security_desc" => "谁可以访问这个页面。",
        "security_title" => "仅允许",
        "session" => "会话",
        "session_desc" => "将用户会话添加到页面，可以限制页访问。",
        "stop_impersonate_success" => "",
        "users" => "注册用户"
    ],
    "settings" => [
        "activate_mode" => "激活模式",
        "activate_mode_admin" => "管理员",
        "activate_mode_admin_comment" => "管理员激活.",
        "activate_mode_auto" => "自动",
        "activate_mode_auto_comment" => "注册成功后自动激活.",
        "activate_mode_comment" => "选择激活方式.",
        "activate_mode_user" => "用户",
        "activate_mode_user_comment" => "邮件激活.",
        "activation_tab" => "激活",
        "allow_registration" => "允许用户注册",
        "allow_registration_comment" => "如果禁用注册，用户只能由管理员创建。",
        "block_persistence" => "",
        "block_persistence_comment" => "",
        "default_country" => "",
        "default_country_comment" => "",
        "default_state" => "",
        "default_state_comment" => "",
        "hint_templates" => "你可以通过管理面板 邮件 > 邮件模版 设置邮件正文模版.",
        "location_tab" => "",
        "login_attribute" => "登录字段",
        "login_attribute_comment" => "选择用户登录类型.",
        "menu_description" => "管理用户选项.",
        "menu_label" => "用户选项",
        "min_password_length" => "",
        "min_password_length_comment" => "",
        "no_mail_template" => "不发送邮件提醒",
        "notifications_tab" => "提醒",
        "profile_tab" => "",
        "registration_tab" => "注册",
        "remember_always" => "",
        "remember_ask" => "",
        "remember_login" => "",
        "remember_login_comment" => "",
        "remember_never" => "",
        "require_activation" => "选项",
        "require_activation_comment" => "用户必须激活后才能登录.",
        "signin_tab" => "登录",
        "use_register_throttle" => "",
        "use_register_throttle_comment" => "",
        "use_throttle" => "登录限制",
        "use_throttle_comment" => "用户重复登录失败时禁用用户.",
        "users" => "用户",
        "welcome_template" => "欢迎模版",
        "welcome_template_comment" => "用户激活成功后邮件模版."
    ],
    "state" => [
        "code" => "",
        "code_comment" => "",
        "label" => "",
        "name" => "",
        "name_comment" => ""
    ],
    "user" => [
        "account" => "帐号",
        "avatar" => "头像",
        "block_mail" => "阻止向该用户发送邮件",
        "confirm_password" => "确认密码",
        "confirm_password_comment" => "再次输入密码.",
        "create_password" => "创建密码",
        "create_password_comment" => "输入登陆新密码",
        "created_at" => "注册时间",
        "created_ip_address" => "",
        "details" => "描述",
        "email" => "邮箱",
        "empty_groups" => "无可用用户群组",
        "groups" => "群组",
        "id" => "ID",
        "is_guest" => "访客",
        "is_offline" => "当前离线",
        "is_online" => "当前在线",
        "joined" => "加入",
        "label" => "用户",
        "last_ip_address" => "",
        "last_seen" => "上次登录时间",
        "name" => "名",
        "name_empty" => "匿名的",
        "reset_password" => "重置密码",
        "reset_password_comment" => "请输入新密码.",
        "send_invite" => "用邮件发送邀请",
        "send_invite_comment" => "发送一个包含用户名和密码的欢迎消息",
        "status_activated" => "活跃的",
        "status_guest" => "访客",
        "status_registered" => "在册的",
        "surname" => "姓",
        "user" => "",
        "username" => "用户名",
        "username_placeholder" => "输入你的用户名"
    ],
    "users" => [
        "activate_confirm" => "确认激活该用户?",
        "activate_manually" => "手动激活该用户",
        "activate_selected" => "",
        "activate_selected_confirm" => "",
        "activate_selected_empty" => "",
        "activate_selected_success" => "",
        "activate_warning_desc" => "用户未激活,无法登录.",
        "activate_warning_title" => "用户未激活!",
        "activated_success" => "用户激活成功!",
        "activating" => "激活中...",
        "active_manually" => "",
        "all_users" => "所有用户",
        "ban_selected" => "禁用选中",
        "ban_selected_confirm" => "确定禁止选中的用户吗？",
        "ban_selected_empty" => "没有选中的用户需要禁用。",
        "ban_selected_success" => "成功禁用选中用户",
        "banned_hint_desc" => "该用户已被管理员禁止，无法登录。",
        "banned_hint_title" => "用户已被禁止",
        "bulk_actions" => "批量操作",
        "convert_guest_confirm" => "转换访客为用户吗？",
        "convert_guest_manually" => "转换为注册用户",
        "convert_guest_success" => "用户已转换为注册帐户",
        "deactivate_selected" => "停用选中",
        "deactivate_selected_confirm" => "确定停用选中的用户吗？",
        "deactivate_selected_empty" => "没有选中任何要停用的用户。",
        "deactivate_selected_success" => "成功停用选定的用户。",
        "delete_confirm" => "确认删除该用户?",
        "delete_selected" => "删除选中",
        "delete_selected_confirm" => "删除所选用户?",
        "delete_selected_empty" => "未选择任何用户.",
        "delete_selected_success" => "成功删除所选用户.",
        "guest_hint_desc" => "此用户仅供参考，在登录前需要注册。",
        "guest_hint_title" => "这是一个游客用户",
        "impersonate_confirm" => "",
        "impersonate_success" => "",
        "impersonate_user" => "",
        "list_title" => "管理用户",
        "menu_label" => "用户",
        "new_user" => "添加新用户",
        "restore_selected" => "恢复选中",
        "restore_selected_confirm" => "确定恢复选中的用户吗？",
        "restore_selected_empty" => "没有选中任何要恢复的用户。",
        "restore_selected_success" => "成功恢复选中选中的用户。",
        "return_to_list" => "返回用户列表",
        "trashed_hint_desc" => "此用户已停用其帐户，不再希望出现在网站上。他们可以在任何时候通过重新登录来恢复帐户。",
        "trashed_hint_title" => "用户已停用其帐户",
        "unban_confirm" => "确定解禁该用户吗？",
        "unban_selected" => "启用选中",
        "unban_selected_confirm" => "确定启用选中的用户吗?",
        "unban_selected_empty" => "没有选中的用户可以启用。",
        "unban_selected_success" => "成功启用选中的用户。",
        "unban_user" => "解禁用户",
        "unbanned_success" => "用户已经解禁",
        "unsuspend" => "",
        "unsuspend_confirm" => "",
        "unsuspend_success" => "",
        "update_details" => "更新详细信息"
    ]
];
